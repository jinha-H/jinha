...
## Ch.7
* **7-1** Rectangle 객체 생성 및 유효성 검사 메서드 활용
* **7-2** 배열의 프로토타입을 상속받는 Grade 객체 생성
* **7-3** 배열의 프로토타입을 상속받는 Grade 객체의 length 속성 삭제 후 동작 변화
* **7-4** 배열 리터럴로 프로토타입을 설정한 Grade 객체의 length 속성 삭제 후 동작 변화 : 이 코드는 이전 예제와 유사하지만, Grade.prototype을 빈 배열 대신 배열 리터럴 ['a', 'b', 'c', 'd'] 로 설정한 점이 다름.
* **7-5** Rectangle과 Square 객체를 생성하여 넓이를 계산하는 코드
* **7-6** 사각형과 정사각형의 넓이를 계산하는 두 가지 생성자 함수(Rectangle, Square)를 정의하고 각각의 객체를 생성하여 넓이를 출력(height 속성을 명시적으로 추가)
* **7-7** 사각형(Rectangle)을 나타내는 생성자 함수와 이를 상속받아 정사각형(Square)을 나타내는 생성자 함수를 정의하고, 각각의 객체를 생성하여 넓이를 계산
* **7-8** extendClass1 함수를 사용하여 Rectangle 클래스를 상속받는 Square 클래스 정의 및 넓이 계산
* **7-9** extendClass2 즉시 실행 함수를 사용하여 프로토타입 상속 구현 및 Square : 클래스 상속 구현 방식이 extendClass2라는 즉시 실행 함수를 사용하는 점이 다름
* **7-10** 사각형(Rectangle)을 나타내는 생성자 함수와 이를 상속받아 정사각형(Square)을 나타내는 생성자 함수를 정의하고, Object.create를 사용하여 프로토타입 상속을 구현
* **7-11** 자바스크립트에서 프로토타입 기반 상속을 구현하는 extendClass1 함수를 정의. 이 함수를 사용하면 SuperClass(부모 클래스)를 상속받아 SubClass(자식 클래스)를 생성하고, 필요에 따라 subMethods 객체를 통해 자식 클래스에 추가적인 메서드를 정의할 수 있음.
* **7-12** 자바스크립트에서 프로토타입 기반 상속을 구현하는 extendClass2 함수를 즉시 실행 함수(IIFE) 패턴으로 정의.
* **7-13** 프로토타입 기반 상속을 구현하는 extendClass3 함수를 정의
* **7-14** 프로토타입 기반 상속을 구현하는 extendClass 함수를 정의하고, super 메서드를 추가하여 자식 클래스에서 부모 클래스의 메서드에 접근
* **7-15** ES5와 ES6의 클래스 정의 방식과 인스턴스 메서드, 정적 메서드 사용법을 보여줌.
* **7-16** ES6의 클래스 문법을 사용하여 사각형(Rectangle) 클래스와 이를 상속받는 정사각형(Square) 클래스를 정의
